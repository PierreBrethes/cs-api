...
async function create(data, req, res) {
    ...
    const endpoint = () => {
        return "feed/create?auth=5c5d73498a93f8847e8b4567.enufbAIrdblkaG9zvd&url=https://instagram.com/" + data.name
    };
    try {
            ...
            const result = (await Spyinglist
                    .findOrCreate({
                        where: {
                            ...
                            name: data.name,
                            hashrss: fetchrss.feed.rss_url,
                            id_fetchrss: fetchrss.feed.id
                        }
                    })
                    .spread((item, created) => {
                        if (created === true) {
...
async function deleteItem(data, req, res) {
    const host = "http://fetchrss.com/api/v1/";
    const endpoint = () => {
        return "feed/delete?auth=5c5d73498a93f8847e8b4567.enufbAIrdblkaG9zvd&id=" + data.id_fetchrss
    };
    const fetchrss = await request(host, endpoint(), null, null, "GET");
